odoo.define('payment_conekta.conekta', function(require) {
    "use strict";
    ...
    var _t = core._t;
    var qweb = core.qweb;
    ajax.loadXML('/payment_conekta/static/src/xml/conekta_templates.xml', qweb);

    // The following currencies are integer only, see
    // https://conekta.com/docs/currencies#zero-decimal
    var int_currencies = [
    ...
    if ($.blockUI) {
        ...
        $.blockUI.defaults.baseZ = 2147483647; //same z-index as conektaCheckout
        $.blockUI.defaults.css.border = '0';
        $.blockUI.defaults.css["background-color"] = '';
        $.blockUI.defaults.overlayCSS["opacity"] = '0.9';
    }
    function getconektaHandler()
    {
        var handler = conektaCheckout.configure({
            key: $("input[name='conekta_key']").val(),
            // image: $("input[name='conekta_image']").val(),
            locale: 'auto',
            token: function(token, args) {
                ...
                ajax.jsonRpc("/payment/conekta/create_charge", 'call', {
                    ...
                    acquirer_id: $("#acquirer_conekta").val(),
                    ...
                    var wizard = $(qweb.render('conekta.error', {'msg': msg || _t('Payment error')}));
    ...
    var observer = new MutationObserver(function(mutations, observer) {
        for(var i=0; i<mutations.length; ++i) {
            for(var j=0; j<mutations[i].addedNodes.length; ++j) {
                if(mutations[i].addedNodes[j].tagName.toLowerCase() === "form" && mutations[i].addedNodes[j].getAttribute('provider') == 'conekta') {
                    display_conekta_form($(mutations[i].addedNodes[j]));
                }
            }
        }
    });
    ...
    function display_conekta_form(provider_form) {
        ...
        var acquirer_id = payment_form.find('input[type="radio"][data-provider="conekta"]:checked').data('acquirer-id');
        if (! acquirer_id) {
            return false;
        }
    ...
    $.getScript("https://checkout.conekta.com/checkout.js", function(data, textStatus, jqxhr) {
        observer.observe(document.body, {childList: true});
        display_conekta_form($('form[provider="conekta"]'));
    });
});
